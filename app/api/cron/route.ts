import { getEmailNotifType, searchForHighest, searchForLowest } from "@/lib/utils";
...
export async function GET(request: Request) {
    try {
        ...
        const updatedProducts = await Promise.all(
            allProducts.map(async (product: { id: string; trackers: string[] } & item) => {
                // scrape each product
                ...
                if (product.priceHistory.at(-1) === scrapedProduct.currentPrice) return;

                // update products price
                const updatedPriceHistory = [...product.priceHistory, scrapedProduct.currentPrice];
                product = {
                    ...scrapedProduct,
                    id: product.id,
                ...
                const emailNotificationType = getEmailNotifType(scrapedProduct, updatedProduct);
                ...
                if (emailNotificationType || updatedProduct.trackers.length > 0) {
                    const emailContent = await generateEmailBody({ url: updatedProduct.url, title: updatedProduct.title }, emailNotificationType);
                    if (emailContent) await sendEmail(emailContent, updatedProduct.trackers);
                }
            })
        );
